...
Building WSO2 Application Server Docker images using Puppet for Kubernetes:
  ...
  5. Copy the WSO2 Application Server 5.3.0 product pack file to `PUPPET_HOME/modules/wso2as/files` location (Note that if you use a different product version, the `-v` flag provided to the subsequent scripts have to be changed to match).
