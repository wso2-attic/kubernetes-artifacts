...
function default {
    ...
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2as" "default" "${default_port}"
}
...
function distributed {
    ...
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2as" "manager" "${manager_port}" && \

    bash "${common_scripts_folder}/deploy-kubernetes-rc.sh" "wso2as" "worker" && \
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2as" "worker" "${worker_port}"
}
