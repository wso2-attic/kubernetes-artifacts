...
function default {
  ...
  bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2esb" "default" "${default_port}"
}
...
function distributed {
    ...
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2esb" "manager" "${manager_port}" && \

    bash "${common_scripts_folder}/deploy-kubernetes-rc.sh" "wso2esb" "worker" && \
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2esb" "worker" "${worker_port}"
}
