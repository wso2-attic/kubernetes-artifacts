...
function default {
  ...
  bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2dss" "default" "${default_port}"
}
...
function distributed {
    ...
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2dss" "manager" "${manager_port}" && \

    bash "${common_scripts_folder}/deploy-kubernetes-rc.sh" "wso2dss" "worker" && \
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2dss" "worker" "${worker_port}"
}
