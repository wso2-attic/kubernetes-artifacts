# Kubernetes Artifacts for WSO2 Identity Server Key Manager#
...
Building WSO2 Identity Server Key Manager Docker images using Puppet for Kubernetes:
  ...
  2. Copy the [dependency jars](https://docs.wso2.com/display/KA100/Kubernetes+Membership+Scheme+for+WSO2+Carbon) for clustering to `PUPPET_HOME/modules/wso2is-km/files/configs/repository/components/lib` location.
  3. Copy  [`kernel patch0005`](http://product-dist.wso2.com/downloads/carbon/4.4.1/patch0005/WSO2-CARBON-PATCH-4.4.1-0005.zip) to `PUPPET_HOME/modules/wso2is-km/files/patches/repository/components/patches` folder.
  ...
  4. Copy the [`mysql-connector-java-5.1.36-bin.jar`](http://mvnrepository.com/artifact/mysql/mysql-connector-java/5.1.36) file to `PUPPET_HOME/modules/wso2is-km/files/configs/repository/components/lib` location.
  5. Copy the WSO2 Identity Server 5.1.0 product distribution pre-packaged with Key Manager feature to `PUPPET_HOME/modules/wso2is-km/files` location (Note that if you use a different product version, the `-v` flag provided to the subsequent scripts have to be changed to match).
