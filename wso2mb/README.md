...
To deploy WSO2 Message Broker on Kubernetes, the following steps have to be done.
* Build WSO2 Message Broker Docker images
* Copy the images to the Kubernetes Nodes
* Run `deploy.sh` which will deploy the Service and the Replication Controllers

#### 1. Build Docker Images

To manage configurations and artifacts when building Docker images, WSO2 recommends to use [`wso2/puppet modules`](https://github.com/wso2/puppet-modules) as the provisioning method. A specific data set for Kubernetes platform is available in WSO2 Puppet Modules. It's possible to use this data set to build Dockerfiles for WSO2 Message Broker for Kubernetes with minimum configuration changes.

Building WSO2 Message Broker Docker images using Puppet for Kubernetes:

  1. Clone `wso2/puppet modules` and `wso2/dockerfiles` repositories (alternatively you can download the released artifacts using the release page of the GitHub repository).
  2. Copy the [dependency jars](https://docs.wso2.com/display/KA100/Kubernetes+Membership+Scheme+for+WSO2+Carbon) for clustering to `PUPPET_HOME/modules/wso2mb/files/configs/repository/components/lib` location.
  3. Copy  [`kernel patch0005`](http://product-dist.wso2.com/downloads/carbon/4.4.1/patch0005/WSO2-CARBON-PATCH-4.4.1-0005.zip) to `PUPPET_HOME/modules/wso2mb/files/patches/repository/components/patches` folder.
  ...
  4. Copy the [`mysql-connector-java-5.1.36-bin.jar`](http://mvnrepository.com/artifact/mysql/mysql-connector-java/5.1.36) file to `PUPPET_HOME/modules/wso2mb/files/configs/repository/components/lib` location.
  5. Copy the WSO2 Message Broker 3.1.0 product pack file to `PUPPET_HOME/modules/wso2mb/files` location (Note that if you use a different product version, the `-v` flag provided to the subsequent scripts have to be changed to match).
  ...
  8. Build the Dockerfile with the following command:
    ...
    **`./build.sh -v 3.1.0 -s kubernetes`**

  Note that `-s kubernetes` flag denotes the Kubernetes platform, when it comes to selecting the configuration from Puppet.

  This will build the default profile of WSO2 Message Broker 3.1.0 for Kubernetes platform, using configuration specified in Puppet.
...
#### 3. Deploy Kubernetes Artifacts
  ...
  2. run the deploy.sh script:
    ...
    **`./deploy.sh`**
      ...
      This will deploy the WSO2 Message Broker 3.1.0 default profile in Kubernetes, using the image available in Kubernetes nodes, and notify once the intended service starts running on the pod.
...
#### 4. Access Management Console
  1. Add an host entry (in Linux, using the `/etc/hosts` file) for `wso2mb-default`, resolving to the Kubernetes node IP.
...
#### 5. Undeploying
  ...
  2. run the `undeploy.sh` script:
    ...
    **`./undeploy.sh`**
      ...
      This will undeploy the WSO2 Message Broker specific DB pod (`mysql-mbdb`), Kubernetes Replication Controllers, and Kubernetes services. Additionally if `-f` flag is provided when running `undeploy.sh`, it will also undeploy the shared Governance and User DB pods, Replication Controllers, and Services.
      **`./undeploy.sh -f`**

For more detailed instructions on deploying WSO2 Message Broker on Kubernetes, please refer the wiki links under the Documentation section below.
