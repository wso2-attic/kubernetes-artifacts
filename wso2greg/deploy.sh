...
function default {
  ...
  bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2greg" "default" "${default_port}"
}
...
function distributed {
    ...
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2greg" "store" "${store_port}" && \

    bash "${common_scripts_folder}/deploy-kubernetes-rc.sh" "wso2greg" "publisher" && \
    bash "${common_scripts_folder}/wait-until-server-starts.sh" "wso2greg" "publisher" "${publisher_port}"
}
